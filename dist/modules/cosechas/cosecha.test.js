...
const cosechasService = __importStar(require("./cosechas.service"));
...
describe("Pruebas de Integración - Módulo Cosechas", () => {
    ...
    describe("GET /api/cosechas", () => {
        it("debe rechazar la petición con status 401 si no se envía token", async () => {
            const res = await (0, supertest_1.default)(app_1.default).get("/api/cosechas");
    ...
    describe("POST /api/cosechas", () => {
        it("debe rechazar la petición con status 401 si no se envía token", async () => {
            const res = await (0, supertest_1.default)(app_1.default).post("/api/cosechas").send({
                fecha: "2026-07-27",
                kilosCosechados: 500,
        ...
        it("debe crear una cosecha exitosamente vinculada a un trabajador con token válido", async () => {
            ...
            cosechasService.crearCosecha.mockResolvedValue(mockCosecha);
...
//# sourceMappingURL=cosecha.test.js.map